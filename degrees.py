    ...
    while not frontier.empty():
        ...
        for movie_id, neighbor_id in neighbors_for_person(person_id):
            if neighbor_id not in explored and not frontier.contains_state(neighbor_id):
                frontier.add(Node(neighbor_id, node, (movie_id, neighbor_id)))
                # Build the path to this neighbor
                paths[neighbor_id] = paths[person_id] + [(movie_id, neighbor_id)]
                # If the neighbor is the target, return the path immediately
                if neighbor_id == target:
                    return paths[neighbor_id]
